...
def number_films_in_category(category_id:int)->pd.DataFrame:
    ...
    pd.DataFrame: DataFrame zawierający wyniki zapytania
    '''
    df = pd.read_sql(f"SELECT category.name, COUNT(*) FROM category INNER JOIN film_category ON category.category_id = film_category.category_id WHERE category.category_id = {category_id} GROUP BY category.name",con=connection)
    return df
...
def client_from_city(city:str)->pd.DataFrame:
    ...
    pd.DataFrame: DataFrame zawierający wyniki zapytania
    '''
    df = pd.read_sql(f"SELECT first_name, last_name FROM customer WHERE address_id IN (SELECT address.address_id FROM address INNER JOIN city ON city.city_id = address.city_id WHERE city.city = {city}) ORDER BY first_name, last_name",con=connection)
    return df
